...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['admin_remark'])) {
        ...
        $update_query = "UPDATE contact_us SET admin_remark='$admin_remark' WHERE id=$id";
        $update_result = mysqli_query($conn, $update_query);

        if ($update_result) {
            echo "<script>
            alert('Admin remark updated successfully');
            window.location.href='read-query.php';
            </script>";
        } else {
            echo "<script>alert('Failed to update admin remark');</script>";
        }
    }

    // Fetch the updated query details again
    $select_query = "SELECT * FROM contact_us WHERE id = $id";
    ...
    if ($execute_query && mysqli_num_rows($execute_query) > 0) {
        ...
        $is_remark_given = !empty($admin_remark);
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-10">
                ...
                <div class="p-3">
                    <div class="dashboard-header mt-3">ADMIN | ADD QUERY DETAILS</div>
                ...
                <div class="container">
                    <span>Manage:<span>Query details</span></span>
                    <form method="post">
                        <div class="row mb-2">
                            <label class="col-sm-2 col-form-label">Full Name</label>
                            <div class="col-sm-10">
                                <input type="text" readonly class="form-control-plaintext" value="<?php echo htmlspecialchars($name); ?>" />
                            </div>
                        </div>
                        <div class="row mb-2">
                            <label class="col-sm-2 col-form-label">Email Id</label>
                            <div class="col-sm-10">
                                <input type="text" readonly class="form-control-plaintext" value="<?php echo htmlspecialchars($email); ?>" />
                            </div>
                        </div>
                        <div class="row mb-2">
                            <label class="col-sm-2 col-form-label">Contact Number</label>
                            <div class="col-sm-10">
                                <input type="text" readonly class="form-control-plaintext" value="<?php echo htmlspecialchars($contact); ?>" />
                            </div>
                        </div>
                        <div class="row mb-2">
                            <label class="col-sm-2 col-form-label">Message</label>
                            <div class="col-sm-10">
                                <input type="text" readonly class="form-control-plaintext" value="<?php echo htmlspecialchars($message); ?>" />
                            </div>
                        </div>
                        <div class="row mb-2">
                            <label class="col-sm-2 col-form-label">Query Date</label>
                            <div class="col-sm-10">
                                <input type="text" readonly class="form-control-plaintext" value="<?php echo htmlspecialchars($query_date); ?>" />
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label for="adminRemark" class="col-sm-2 col-form-label">Admin Remark</label>
                            <div class="col-sm-10">
                                <textarea id="adminRemark" name="admin_remark" class="form-control" rows="3" placeholder="Enter admin remarks"><?php echo htmlspecialchars($admin_remark); ?></textarea>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-10 offset-sm-2">
                                <button type="submit" class="btn btn-primary"<?php echo $is_remark_given ? 'disabled' : ''; ?>>
